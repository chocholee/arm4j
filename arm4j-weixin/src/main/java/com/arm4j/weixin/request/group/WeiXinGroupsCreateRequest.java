...
public class WeiXinGroupsCreateRequest {

    public static GroupEntity request(String accessToken, GroupEntity group) throws WeiXinRequestException {
        Map<String, Object> bodyMap = new HashMap<>();
        bodyMap.put("group", group);

        // 发送请求
        String result = WeiXinCoreManagement.getInstance().get(WeiXinToken.GROUPS_CREATE)
                .createConn()
                .connect(
        ...
        if (!StringUtils.isEmpty(result)) {
            ...
            if (!resultJSON.containsKey("errcode")) {
                return JSON.parseObject(resultJSON.getString("group"), GroupEntity.class);
            } else {
